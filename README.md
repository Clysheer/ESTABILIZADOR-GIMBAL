...
- **Giroscopio**: mide la velocidad angular, es decir, la rotación de la cámara en cada uno de los ejes.

La información de estos dos sensores se procesa para calcular la orientación actual de la cámara. La combinación de ambos datos (acelerómetro y giroscopio) permite tener una estimación más precisa del movimiento, ya que cada sensor compensa las limitaciones del otro.
...
- **Proporcional (P)**: Compara la diferencia (error) entre el ángulo actual y el deseado. Cuanto mayor es el error, mayor será la corrección aplicada.
- **Integral (I)**: Suma el error a lo largo del tiempo, lo que ayuda a corregir pequeños desajustes acumulados.
...
![Diagrama de flujo](https://github.com/user-attachments/assets/1b4291a9-5131-499f-83e9-1309a1407efe)

### Descripción detallada de cada bloque

#### 3.1. Inicialización del sistema
En esta etapa se configuran:
- La comunicación I2C (Wire.h)
...
- Los datos estables del acelerómetro
- La rapidez y suavidad del giroscopio
...
- D: suaviza y evita oscilaciones

El PID entrega un valor corregido que indica cuánto debe moverse cada servo.
...
#### 3.7. Loop principal
Se ejecuta unas 200 a 400 veces por segundo, realizando:
1. Lectura MPU6050
...
### Flujo de funcionamiento del software (texto para informe)
1. El sistema inicia y configura todos los módulos.
2. El microcontrolador lee las mediciones del MPU6050.
3. Los datos se filtran para obtener ángulos limpios.
4. Se calcula la orientación de la cámara.
...
// Setpoints (posición deseada del gimbal)
...
// Factor del filtro complementario (0.98 → giroscopio domina, 0.02 → acelerómetro corrige)
...
void setup() {
    ...
    // Calibrar giroscopio (promedio en reposo)
    ...
    gyroXoffset = gxSum / 2000.0;
    gyroYoffset = gySum / 2000.0;
    gyroZoffset = gzSum / 2000.0;
...
void loop() {
    ...
    int servoRollVal  = constrain(90 + outputRoll, 0, 180);
    int servoPitchVal = constrain(90 + outputPitch, 0, 180);
    int servoYawVal   = constrain(90 + outputYaw, 0, 180);
    
    servoRoll.write(servoRollVal);
    servoPitch.write(servoPitchVal);
    servoYaw.write(servoYawVal);
    
    // Debug
    Serial.print("Roll: "); Serial.print(roll);
    Serial.print(" | Pitch: "); Serial.print(pitch);
    Serial.print(" | Yaw: "); Serial.print(yaw);
    Serial.print(" | S_R: "); Serial.print(servoRollVal);
    Serial.print(" | S_P: "); Serial.print(servoPitchVal);
    Serial.print(" | S_Y: "); Serial.println(servoYawVal);
...
- Fusible polímero o PTC en línea de batería (recomendado)

Aplicación: el esquema se usa para fabricar la PCB que interconecta todos los módulos con vías limpias, anclajes mecánicos para el ensamblado y header para conexión de la estructura 3D.
...
- Pruebas: leer registros WHO_AM_I, comprobar aceleraciones en reposo.
- Aplicación: proveer mediciones de aceleración y velocidad angular.
...
![Especificaciones técnicas](https://github.com/user-attachments/assets/f7fc629e-27c9-45c5-8f98-e32e1fc26220)
...
- Perímetro: 3 shells
- Temp extrusor: 200–210 °C (según filamento)
- Temp cama: 50–60 °C
...
![Diseño Final](https://github.com/user-attachments/assets/a3b45236-305d-488f-9a98-a2b382f607ed)

### Proceso de diseño
![Proceso de diseño](https://github.com/user-attachments/assets/e8c8b42c-d206-4bc8-97fd-f431d634465c)

### Primer Prototipo
![Primer Prototipo](https://github.com/user-attachments/assets/0819e72e-3f33-4f94-bd6e-0297b10f5c93)
...
- Posibilidad de transmisión de datos, telemetría o configuración desde una app.

Además, el mayor rendimiento del ESP32 permitiría implementar estabilización en tiempo real con menor latencia.
...
- Aumentar la rigidez de la estructura para reducir resonancias.
- Integrar directamente soportes para cableado interno.
...
2. Verificar que el LED del Arduino encienda.
3. Esperar 2–3 segundos para que el MPU6050 se inicialice.
...
- Ángulo deseado (0°)
- Ángulo real medido por el MPU6050

y ajusta los servos inmediatamente.
...
|----------|----------------|----------|
| El gimbal vibra demasiado | Ganancias PID incorrectas | Ajustar Kp, Ki o Kd |
| Se inclina hacia un lado | Calibración incorrecta | Reiniciar con el gimbal quieto |
| Los servos hacen ruido continuo | Demasiado esfuerzo mecánico | Recentrar la cámara |
| No enciende | Sin alimentación | Revisar batería y LM2596 |
| Movimientos lentos | Servo dañado o bajo torque | Reemplazar servo SG90 |
